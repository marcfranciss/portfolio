    ...
    <section id="blog" className="max-w-7xl mx-auto px-5 sm:px-8 py-10">
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
        {POSTS.map(({ date, read, title, accent }) => (
          ...
          >
            <div className={`h-32 bg-gradient-to-br ${accent} flex items-center justify-center`}>
              <Cloud className="w-9 h-9 text-white/80" />
            </div>
            <div className="p-5">
              <h3 className={`font-semibold mb-3 leading-snug ${dark ? "text-white" : "text-slate-900"}`}>
                {title}
              </h3>
